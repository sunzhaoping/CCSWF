...
// Grid-based container for boxes.
{
		...
		m_query_id(0)
	{
		...
		// Allocate the grid.
		m_grid = new gameswf::array<grid_entry_t*>[x_cells * y_cells];
	}
		...
		m_query_id(0)
	{
		...
		assert(bound.min.x <= bound.max.x);
		assert(bound.min.y <= bound.max.y);
		m_grid = new gameswf::array<grid_entry_t*>[m_x_cells * m_y_cells];
	}
	...
	~grid_index_box()
	{
		...
		// repeated many times).
		gameswf::array<grid_entry_t*>&	cell_array = m_grid[0];
	...
	struct iterator
	{
		...
		// element, and return true.
		{
			...
			gameswf::array<grid_entry_t*>*	cell_array = m_index->get_cell(m_current_cell_x, m_current_cell_y);
	...
	iterator	begin(const box_t& q)
	{
		...
		if (m_query_id == 0)
		{
			...
			{
				gameswf::array<grid_entry_t*>*	cell_array = &m_grid[i];
	...
	// Removes the entry from the index, and deletes it.
	{
		...
		for (int iy = ib.min.y; iy <= ib.max.y; iy++)
		{
			...
			{
				gameswf::array<grid_entry_t*>*	cell_array = get_cell(ix, iy);
	...
	// Should be relatively quick, assuming payload is unique.
	{
		...
		gameswf::array<grid_entry_t*>*	cell_array = get_cell(ip.x, ip.y);
...
private:
	...
	gameswf::array<grid_entry_t*>*	get_cell(int x, int y)
	{
		assert(x >= 0 && x < m_x_cells);
		assert(y >= 0 && y < m_y_cells);
...
//data:
	...
	int	m_y_cells;
	int	m_query_id;
	gameswf::array<grid_entry_t*>*	m_grid;
};
