...
//	 gameswf::array<coord_t>* m_results;
	trilist_accepter* m_output;
	 gameswf::array<poly_vert> m_verts;
	 gameswf::array<path_info> m_input_paths;
	...
	int m_debug_halt_step;
	 gameswf::array<coord_t>* m_debug_edges;
};
...
struct vert_index_sorter {
	vert_index_sorter(const  gameswf::array<poly_vert>& verts)
		: m_verts(verts) {
	}
	...
	const  gameswf::array<poly_vert>& m_verts;
};
...
{
	// Sort.
	 gameswf::array<poly_vert> verts = ts->m_verts;
	 gameswf::array<int> vert_indices(verts.size());   // verts[vert_indices[0]] --> leftmost vert
...
static void init(tristate* ts, trilist_accepter* output, path_supplier* input,
		 /* gameswf::array<coord_t>* results, int path_count, const  gameswf::array<coord_t> paths[], */
		 int debug_halt_step,  gameswf::array<coord_t>* debug_edges)
...
static void debug_make_x(gameswf::array<coord_t>* out, const vec2_t& v)
{
	if (!out) return;
...
static void debug_make_plus(gameswf::array<coord_t>* out, const vec2_t& v)
{
	if (!out) return;
	out->push_back(v.x);
	out->push_back(v.y - 200);
...
static void debug_make_square(gameswf::array<coord_t>* out, const vec2_t& v)
{
	if (!out) return;
	out->push_back(v.x - 200);
	out->push_back(v.y - 200);
...
{
	ear_clip_array_io(gameswf::array<coord_t>* results, int path_count, const  gameswf::array<coord_t> paths[])
		:
		m_results(results),
...
// data:
	 gameswf::array<coord_t>* m_results;
	int m_path_count;
	const  gameswf::array<coord_t>* m_paths;
	int m_next_path;
};
