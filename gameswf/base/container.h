...
//  gameswf::array<> is much like std::vector<>
//
...
public:
	...
	void	append(const  gameswf::array<T>& other)
...
template<typename T, typename U, typename hash_functor = fixed_size_hash<T> >
...
// //  gameswf::tu_string is a subset of std::string, for the most part
// class  gameswf::tu_string : public std::string
// {
// public:
// 	 gameswf::tu_string(const char* str) : std::string(str) {}
// 	 gameswf::tu_string() : std::string() {}
// 	 gameswf::tu_string(const  gameswf::tu_string& str) : std::string(str) {}
...
// template<class U>
// class string_hash : public hash< gameswf::tu_string, U, std::hash<std::string> >
// {
// };
...
//		assert(&val < &m_buffer[0] || &val > &m_buffer[m_buffer_size]);
	...
	void	operator=(const  gameswf::array<T>& a)
	...
	void	transfer_members(gameswf::array<T>* a)
...
public:
	...
	tu_string(const  gameswf::tu_string& str)
	{
		m_union.m_local.m_bufsize_minus_length = sizeof(m_union.m_local.m_buffer);
		m_union.m_local.m_buffer[0] = 0;
	...
	exported_module void	operator=(const  gameswf::tu_string& str)
	{
		if (this != &str) {
			resize(str.size());
			memcpy(get_buffer(), str.get_buffer(), size() + 1);
	...
	exported_module bool	operator==(const  gameswf::tu_string& str) const
	{
		if (size() != str.size()) {
			return false;
		}
	...
	{
		return (*(const_cast< gameswf::tu_string*>(this)))[index];
	}
	...
	exported_module  gameswf::tu_string	operator+(const  gameswf::tu_string& str) const
	{
		 gameswf::tu_string	new_string(*this);
		new_string += str;
		return new_string;
	}

	exported_module  gameswf::tu_string	operator+(const char* str) const
	...
	exported_module static void	encode_utf8_from_wchar( gameswf::tu_string* result, const uint32* wstr);
	exported_module static void	encode_utf8_from_wchar( gameswf::tu_string* result, const uint16* wstr);
	...
	exported_module  gameswf::tu_string utf8_to_upper() const;

	// Return the string in this container as all lower case letters
	exported_module  gameswf::tu_string utf8_to_lower() const;
	...
	// Returns a  gameswf::tu_string that's a substring of this.  start and
	...
	exported_module  gameswf::tu_string	utf8_substring(int start, int end) const;
...
private:
	...
	{
		return const_cast< gameswf::tu_string*>(this)->get_buffer();
	}
	...
	// The idea here is that  gameswf::tu_string is an N-byte structure,
...
// Uses  gameswf::tu_string for implementation.
class tu_stringi
{
public:
	tu_stringi() {}
	tu_stringi(const char* str) : m_string(str) {}
	tu_stringi(const  gameswf::tu_string& str) : m_string(str) {}
	...
	void	operator=(const  gameswf::tu_string& str) { m_string = str; }
...
template<typename U>
class string_hash : public hash< gameswf::tu_string, U, string_hash_functor< gameswf::tu_string> >
{
};
...
// Utility: handy sprintf wrapper.
 gameswf::tu_string string_printf(const char* fmt, ...)
#ifdef __GNUC__
