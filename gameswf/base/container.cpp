...
void  gameswf::tu_string::append_wide_char(uint16 c)
{
	char buf[8];
	int index = 0;
...
void  gameswf::tu_string::append_wide_char(uint32 c)
{
	char buf[8];
	int index = 0;
...
void	 gameswf::tu_string::resize(int new_size)
{
	assert(new_size >= 0);
...
/*static*/ void	encode_utf8_from_wchar_generic( gameswf::tu_string* result, const char_type* wstr)
{
...
void  gameswf::tu_string::encode_utf8_from_wchar( gameswf::tu_string* result, const uint32* wstr)
{
	encode_utf8_from_wchar_generic<uint32>(result, wstr);
}


void  gameswf::tu_string::encode_utf8_from_wchar( gameswf::tu_string* result, const uint16* wstr)
{
	encode_utf8_from_wchar_generic<uint16>(result, wstr);
}


/*static*/ int	 gameswf::tu_string::stricmp(const char* a, const char* b)
{
#ifdef _WIN32
	return _stricmp(a, b);
#else
...
uint32	 gameswf::tu_string::utf8_char_at(int index) const
{
 ...
 gameswf::tu_string	 gameswf::tu_string::utf8_to_upper() const
{
	const char*	buf = get_buffer();
 ...
 gameswf::tu_string	 gameswf::tu_string::utf8_to_lower() const
{
	const char*	buf = get_buffer();
	 gameswf::tu_string str;
	for (;;)
...
/*static*/ int	 gameswf::tu_string::utf8_char_count(const char* buf, int buflen)
{
	const char*	p = buf;
	int	length = 0;
 ...
 gameswf::tu_string	 gameswf::tu_string::utf8_substring(int start, int end) const
{
	assert(start <= end);
	...
	return  gameswf::tu_string(start_pointer, int(end_pointer - start_pointer));
}
...
void test_unicode()
{
	 ...
	 gameswf::tu_string::encode_utf8_from_wchar(&a, L"19 character string");
	assert(a.length() == 19);
...
{
#if 1
	printf("sizeof( gameswf::tu_string) == %d\n", sizeof( gameswf::tu_string));

	 gameswf::array< gameswf::tu_string>	storage;
	storage.resize(2);

	 gameswf::tu_string&	a = storage[0];
	 gameswf::tu_string&	b = storage[1];
	a = "test1";
	...
	assert(b.c_str()[0] == 0);
	 ...
	 gameswf::tu_string c = a + b;

	assert(c.length() == a.length());

	c.resize(2);
	assert(c == "te");
	assert(c ==  gameswf::tu_string("te"));

	assert( gameswf::tu_string("fourscore and sevent") == "fourscore and sevent");
	...
	// Test operator+=( gameswf::tu_string)
	a = "abcdef";
	b = "ghijklmnopqrst";
	a += b;
	...
	// Test operator<(const  gameswf::tu_string&)
	a = "abc";
	assert(a <  gameswf::tu_string("def"));
	assert(a <  gameswf::tu_string("abd"));
	assert(!(a <  gameswf::tu_string("aab")));
	assert(!(a <  gameswf::tu_string("aa")));
	assert(!(a <  gameswf::tu_string("aabz")));
	assert(a <  gameswf::tu_string("abcd"));
	...
	b.erase(3, 1);
	assert(b.size() == 13);
	assert(b ==  gameswf::tu_string("abcdef") + a);
	...
	// TODO: unit tests for  gameswf::array<>, string_hash<>
#endif
