...
namespace gameswf
{
	...
	// static
	 gameswf::tu_string get_full_url(const  gameswf::tu_string& workdir, const char* url)
	{
		  gameswf::tu_string fn;
	...
	{
		assert(new_size <=  gameswf::array<as_value>::size());
	...
	void	vm_stack::drop(int count)
	{
		...
		for (int i = m_stack_size, n =  gameswf::array<as_value>::size(); i < n; i++)
		{
	...
	{
		for (int i = 0; i <  gameswf::array<as_value>::size(); i++)
		{
	...
	character* as_environment::load_file(const char* url, const as_value& target_value, int method)
	{
		...
		// is path relative ?
		 gameswf::tu_string file_name = get_full_url(get_player()->get_workdir(), url);
	...
	as_value	as_environment::get_variable(const  gameswf::tu_string& varname, const  gameswf::array<with_stack_entry>& with_stack) const
	// Return the value of the given var, if it's defined.
	{
		// Path lookup rigamarole.
		as_object*	target = get_target();
		 gameswf::tu_string	path;
		 gameswf::tu_string	var;
	...
	as_value	as_environment::get_variable_raw(
		const  gameswf::tu_string& varname,
		const  gameswf::array<with_stack_entry>& with_stack) const
	...
	void as_environment::set_target(as_value& target, character* original_target)
	{
		if (target.is_string())
		{
			 gameswf::tu_string path = target.to_tu_string();
	...
	void	as_environment::set_variable(
		const  gameswf::tu_string& varname,
		const as_value& val,
		const  gameswf::array<with_stack_entry>& with_stack)
	...
	void	as_environment::set_local(const  gameswf::tu_string& varname, const as_value& val)
	...
	void	as_environment::add_local(const  gameswf::tu_string& varname, const as_value& val)
	...
	void	as_environment::declare_local(const  gameswf::tu_string& varname)
	...
	int	as_environment::find_local(const  gameswf::tu_string& varname, bool ignore_barrier) const
	...
	bool	as_environment::parse_path(const  gameswf::tu_string& var_path,  gameswf::tu_string* path,  gameswf::tu_string* var)
