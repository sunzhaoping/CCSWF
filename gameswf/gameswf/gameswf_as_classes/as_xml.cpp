...
namespace gameswf
{
 ...
 gameswf::array<as_object *> _xmlobjs;    // FIXME: hack alert
...
XML::XML( gameswf::tu_string xml_in)
{
#ifdef DEBUG_MEMORY_ALLOCATION
  log_msg("Creating XML data at %p \n", this);
#endif
...
XML::on_event(event_id id)
{
  ...
  const  gameswf::tu_string&	method_name = id.get_function_name();
...
bool
XML::parseXML( gameswf::tu_string xml_in)
{
  bool ret = true;
...
xml_load(const fn_call& fn)
{
  ...
  const  gameswf::tu_string filespec = fn.env->bottom(fn.first_arg_bottom_index).to_string();
  ...
  //env->bottom(first_arg) = ret;
   gameswf::array<with_stack_entry> with_stack;
   gameswf::array<with_stack_entry> dummy_stack;
  ...
  if (fn.nargs > 0) {
    if (fn.env->top(0).get_type() == as_value::STRING) {
      ...
      //log_msg("\tCreated New XML object at %p\n", xml_obj);
       gameswf::tu_string datain = fn.env->top(0).to_tu_string();
...
xml_loaded(const fn_call& fn)
{
  ...
  xml_as_object*	ptr = (xml_as_object*) (as_object*) fn.this_ptr;
  assert(ptr);
   gameswf::tu_string filespec = fn.env->bottom(fn.first_arg_bottom_index).to_string();
