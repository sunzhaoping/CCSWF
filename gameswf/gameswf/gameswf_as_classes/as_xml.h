...
public:
  ...
  XMLNode *firstChild()
  {
   ...
   gameswf::array<XMLNode *>childNodes()
  {
    return _children;
  }  
...
toString() 	XML.toString()
  ...
  char               *_name;
  char               *_value;
   gameswf::array<XMLNode *>   _children;
   gameswf::array<XMLAttr *>   _attributes;
};
...
class XML {
 public:
  ...
  bool parseXML( gameswf::tu_string xml_in); // Parses an XML document into the specified XML object tree.
  ...
  gameswf:: gameswf::tu_string toString();                // Converts the specified node and 
  ...
  //bool _on_event_loaded;
    ...
    XMLNode     *_nodes;
    //     gameswf::array<XMLNode *>  _node_data;
  //    hash<gameswf::event_id, gameswf::as_value>	_event_handlers;
#if 0
     gameswf::array<struct node *> _childNodes; // Read-only; returns an array containing
  ...
  nextSibling;	// Read-only; references the next sibling in the parent node's child list.
   gameswf::tu_string nodeType;	// The type of the specified node (XML element or text node).
