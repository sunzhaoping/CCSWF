...
namespace gameswf
{
	...
	{
		const  gameswf::tu_string& str = fn.this_value.to_tu_string();
	...
	{
		const  gameswf::tu_string& sstr = fn.this_value.to_tu_string();
		...
		else
		{
			...
			fn.result->set_double( gameswf::tu_string::utf8_char_count(str, (int) (p - str)));
		}
	}
	...
	void string_last_index_of(const fn_call& fn)
	{
		...
		if (fn.nargs < 1)
		{
			...
			if (last_hit == NULL) {
				...
				fn.result->set_double( gameswf::tu_string::utf8_char_count(str, (int) (last_hit - str)));
			}
		}
	}
	...
	{
		const  gameswf::tu_string& this_str = fn.this_value.to_tu_string();
		...
		for (int i = 0; i < max_count; )
		{
			...
			if (delimiter.size() == 0)
			{
				...
				if (n == p)
				{
				 ...
				 gameswf::tu_string word(p, (int) (n - p));
				as_value val;
				as_value index(i);
			...
			else
			{
				...
				if (*p == 0 || match)
				{
					// Emit the previous word.
					 gameswf::tu_string word(word_start, int(p - word_start));
					as_value val;
					as_value index(i);
					val.set_tu_string(word);
					arr->set_member(index.to_tu_string(), val);
