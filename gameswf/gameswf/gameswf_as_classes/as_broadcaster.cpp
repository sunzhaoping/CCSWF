...
namespace gameswf
{
	...
	void	as_listener::broadcast(const fn_call& fn)
	{
		...
		// so we iterate through the copy of args
		 gameswf::tu_string event_name = fn.arg(0).to_tu_string();
		...
		while (m_suspended_event.size() > 0)
		{
			...
			// so we iterate through the copy of args
			 gameswf::array<as_value>& arg = m_suspended_event.front();
			 gameswf::tu_string event_name = arg[0].to_tu_string();
