...
namespace gameswf
{
			...
			if (index >=0 && index < asize)
			{
				...
				if (fn.nargs >= 3)
				{
					...
					if (obj)
					{
						...
						{
							as_value val;
							obj->get_member( gameswf::tu_string(i), &val);
							a->insert(index, val);
						}
					}
					else
		...
		if (a && fn.nargs > 0)
		{
			...
			for (int i = 0; i < fn.nargs; i++)
			{
				...
				if (arg)
				{
					...
					{
						as_value val;
						arg->get_member( gameswf::tu_string(j), &val);
						res->push(val);
					}
				}
				else
	...
	void	as_array_slice(const fn_call& fn)
	{
		...
		if (a)
		{
			...
			{
				as_value val;
				if (a->get_member( gameswf::tu_string(i), &val))
				{
					res->push(val);
				}
			}
		...
		if (a)
		{
			for (int i = fn.nargs - 1; i >= 0; i--)
			{
				...
				if (arg)
				{
					...
					{
						as_value val;
						if (arg->get_member( gameswf::tu_string(j), &val))
						{
							a->insert(0, val);
						}
					}
	...
	// Removes the element starting from 'index' and returns the value of that element.
	{
		...
		if (index >= 0 && index < idx.size())
		{
			...
			get_member( gameswf::tu_string(idx[index]), val);

			// shift elements
			for (int i = index + 1; i < idx.size(); i++)
			{
				as_value v;
				get_member( gameswf::tu_string(idx[i]), &v);
				set_member( gameswf::tu_string(idx[i - 1]), v);
			}

			// remove the last element
			erase( gameswf::tu_string(idx[idx.size() - 1]));
		}
		else
		{
			//assert(0);
	...
	// Inserts the element after 'index'
	{
		...
		if (index >= 0 && index < idx.size())
		{
			...
			{
				as_value v;
				get_member( gameswf::tu_string(idx[i - 1]), &v);
				set_member( gameswf::tu_string(idx[i]), v);
			}

			// insert
			set_member( gameswf::tu_string(idx[index]), val);
	...
	void as_array::sort(int options, as_function* compare_function)
	{
		...
		{
			as_value ival;
			get_member( gameswf::tu_string(i), &ival);
			for (int j = i + 1; j < n; j++)
			{
				as_value jval;
				get_member( gameswf::tu_string(j), &jval);
				...
				if (do_swap)
				{
					...
					ival = jval;
					jval = tmp;
					set_member( gameswf::tu_string(i), ival);
					set_member( gameswf::tu_string(j), jval);
				}
			}
		}
	}
