...
namespace gameswf
{
	...
	static hash<int,  gameswf::tu_string> s_netstream_event_level;
	static hash<int,  gameswf::tu_string> s_netstream_event_code;
	...
	void as_netstream::play(const char* url)
	{
		switch (m_status)
		{
			...
			{
				// is path relative ?
				 gameswf::tu_string infile = get_player()->get_workdir();
				if (strstr(url, ":") || url[0] == '/')
				{
					infile = "";
				}
