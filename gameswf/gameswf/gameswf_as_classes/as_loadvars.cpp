...
namespace gameswf
{
	...
	bool	as_loadvars::send_and_load(const char * c_url, as_object * target, const  gameswf::tu_string & method)
	{
		 gameswf::tu_string host, uri;
		...
		m_headers.set("Host", host);
		 ...
		 gameswf::tu_string request_string = create_request( method, uri, true );
	...
	bool	as_loadvars::load(const char * c_url)
	{
		...
		m_headers.set("Host", host);
		 ...
		 gameswf::tu_string request_string = create_request( "GET", uri, true );
	...
	void	as_loadvars::add_header(const  gameswf::tu_string& name, const  gameswf::tu_string& value)
	{
		m_headers.set(name, value);
	}

	 gameswf::tu_string	as_loadvars::override_to_string()
	{
		 gameswf::tu_string information;
		//create information in the form of name=value (urlencoded)
		string_hash< gameswf::tu_string>::iterator it = m_received_values.begin();
		for(bool first = true; it != m_received_values.end(); ++it)
		{
			 gameswf::tu_string name, value;
	...
	void	as_loadvars::decode(const  gameswf::tu_string& query_string)
	{
		char *start, *end;
		...
		while( start < query_string.c_str() + query_string.size() )
		{
			...
			*end=0;
			...
			if( after_name == NULL )
			 ...
			 gameswf::tu_string name =  gameswf::tu_string(start, int(after_name - start));
			 gameswf::tu_string value =  gameswf::tu_string( after_name + 1 ); //Skip the "="
	...
	{
		string_hash< gameswf::tu_string>::iterator it = m_values.find( name.to_tu_string() );
		if( it != m_values.end() )
		{
		...
		string_hash< gameswf::tu_string>::iterator it2 = m_received_values.find( name.to_tu_string() );
	 ...
	 gameswf::tu_string as_loadvars::create_request(const  gameswf::tu_string& method, const  gameswf::tu_string& uri, bool send_data)
	{
		 gameswf::tu_string information;
		//create information in the form of name=value (urlencoded)
		string_hash< gameswf::tu_string>::iterator it = m_values.begin();
		...
		if( method == "POST")
		{
			 gameswf::tu_string request = string_printf( "POST %s HTTP/1.1\r\n", uri.c_str() );
		...
		{
			 gameswf::tu_string request = string_printf( "GET %s?%s HTTP/1.1\r\n", uri.c_str(), information.c_str() );
			request += create_header();
			request += "\r\n";
	 ...
	 gameswf::tu_string 	as_loadvars::create_header()
	{
		 gameswf::tu_string header;

		// Process header
		for( string_hash< gameswf::tu_string>::iterator it2 = m_headers.begin(); it2 != m_headers.end(); ++it2 )
		{
	...
	void	as_loadvars::parse_request(const  gameswf::tu_string& line, request_data& request)
	{
	...
	void	as_loadvars::parse_header(const  gameswf::tu_string& line, request_data& request)
	{
		if( line == "\r\n" )
		{
	...
	void	as_loadvars::parse_content(const  gameswf::tu_string& line, request_data& request)
	{
	...
	bool as_loadvars::parse_url(const char* c_url,  gameswf::tu_string& host,  gameswf::tu_string& uri)
	{
	...
	{
		for (string_hash< gameswf::tu_string>::iterator it = m_received_values.begin(); it != m_received_values.end(); ++it)
		{
