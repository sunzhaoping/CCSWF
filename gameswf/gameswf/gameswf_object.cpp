...
//		}
	...
	bool as_object::watch(const  gameswf::tu_string& name, as_function* callback,
	...
	bool as_object::unwatch(const  gameswf::tu_string& name)
	{
		if (m_watch)
		{
			as_watch watch;
	...
	// for internal using
	{
		 gameswf::tu_string tab;
		dump(tab);
	}

	void as_object::dump( gameswf::tu_string& tabs)
	...
	// Find the object referenced by the given target.
	{
		...
		const  gameswf::tu_string& path = target.to_tu_string();
		if (path.length() == 0)
		{
			return this;
		}
		...
		if (slash)
		{
			 gameswf::tu_string name(path.c_str(), int(slash - path.c_str()));
			get_member(name, &val);
			tar = val.to_object();
			if (tar)	
			{
	...
	static  gameswf::tu_string s_constructor("__constructor__");
