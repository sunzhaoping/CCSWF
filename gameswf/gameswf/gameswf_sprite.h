...
namespace gameswf
{
	...
	struct sprite_instance : public character
	{
		...
		display_list	m_display_list;
		 gameswf::array<action_buffer*>	m_action_list;
		 gameswf::array<action_buffer*>	m_goto_frame_action_list;

		play_state	m_play_state;
		int		m_current_frame;
		bool		m_update_frame;
		 gameswf::array<bool>	m_init_actions_executed;	// a bit-array class would be ideal for this
...
//		virtual bool	has_looped() const { return m_has_looped; }
		...
		void	do_actions();
		virtual void do_actions(const  gameswf::array<action_buffer*>& action_list);
		exported_module void	goto_frame(const  gameswf::tu_string& target_frame);
		exported_module void	goto_frame(int target_frame_number);
		exported_module bool	goto_labeled_frame(const char* label);

		void	display();

		character*	add_display_object( Uint16 character_id, const  gameswf::tu_string& name,
		...
		void	remove_display_object(const  gameswf::tu_string& name);
		...
		character*	clone_display_object(const  gameswf::tu_string& newname, int depth);
		virtual bool	on_event(const event_id& id);
		virtual const char*	call_method_args(const char* method_name, const char* method_arg_fmt, va_list args);
		virtual  gameswf::tu_string	call_method(const char* method_name, as_value * arguments, int argument_count );
		...
		sprite_instance* attach_movie(const  gameswf::tu_string& id, const  gameswf::tu_string name, int depth);
		...
		virtual void dump( gameswf::tu_string& tabs);

		virtual character_def*	find_exported_resource(const  gameswf::tu_string& symbol);
