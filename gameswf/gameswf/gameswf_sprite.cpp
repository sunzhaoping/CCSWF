...
namespace gameswf
{
	...
	void	execute_actions(as_environment* env, const  gameswf::array<action_buffer*>& action_list);
	...
	void sprite_instance::advance(float delta_time)
	{
		...
		if (m_play_state == PLAY)
		{
			...
			{ 
				...
				{
					// affected depths
					const  gameswf::array<execute_tag*>& playlist = m_def->get_playlist(0);
					 gameswf::array<int> affected_depths;
	...
	void sprite_instance::execute_frame_tags(int frame, bool state_only)
	{
		...
		{
			const  gameswf::array<execute_tag*>*	init_actions = m_def->get_init_actions(frame);
		...
		const  gameswf::array<execute_tag*>&	playlist = m_def->get_playlist(frame);
		for (int i = 0; i < playlist.size(); i++)
		{
	...
	execute_tag*	sprite_instance::find_previous_replace_or_add_tag(int frame, int depth, int id)
	{
		...
		{
			const  gameswf::array<execute_tag*>&	playlist = m_def->get_playlist(f);
	...
	void sprite_instance::do_actions(const  gameswf::array<action_buffer*>& action_list)
	{
	...
	void sprite_instance::goto_frame(const  gameswf::tu_string& target_frame)
	{
	...
	character* sprite_instance::add_display_object( Uint16 character_id, const  gameswf::tu_string& name,
		const  gameswf::array<swf_event*>& event_handlers, int depth, bool replace_if_depth_is_occupied,
...
//		ch->on_event(event_id::CONSTRUCT);	// isn't tested
	...
	void sprite_instance::set_variable(const char* path_to_var, const char* new_value)
	{
		...
		if (new_value == NULL)
		{
		 ...
		 gameswf::array<with_stack_entry>	empty_with_stack;
		 gameswf::tu_string	path(path_to_var);
	...
	// frame_spec could be an integer or a string.
	{
		...
		// Execute the actions.
		const  gameswf::array<execute_tag*>&	playlist = m_def->get_playlist(frame_number);
		for (int i = 0; i < playlist.size(); i++)
		{
			execute_tag*	e = playlist[i];
			if (e->is_action_tag())
	...
	character* sprite_instance::clone_display_object(const  gameswf::tu_string& newname, int depth)
	...
	void sprite_instance::remove_display_object(const  gameswf::tu_string& name)
	...
	const char* sprite_instance::call_method_args(const char* method_name, const char* method_arg_fmt, va_list args)
	{
	 ...
	 gameswf::tu_string sprite_instance::call_method(const char* method_name, as_value * arguments, int argument_count )
	{
	...
	void sprite_instance::attach_display_callback(const char* path_to_object, void (*callback)(void*), void* user_ptr)
	{
		assert(m_parent == NULL);	// should only be called on the root movie.
		 ...
		 gameswf::array<with_stack_entry>	dummy;
		as_value	obj = m_as_environment.get_variable( gameswf::tu_string(path_to_object), dummy);
...
//		visited_objects->set(this, true);
	...
	sprite_instance* sprite_instance::attach_movie(const  gameswf::tu_string& id, 
		const  gameswf::tu_string name, 
		int depth)
	{
	...
	void sprite_instance::dump( gameswf::tu_string& tabs)
	{
		tabs += "  ";
	...
	character_def* sprite_instance::find_exported_resource(const  gameswf::tu_string& symbol)
	{
	...
	void sprite_instance::enumerate(as_environment* env)
	{
		...
		for (int i = 0, n = m_display_list.size(); i<n; i++)
		{
			...
			if (ch)
			{
				const  gameswf::tu_string& name = ch->get_name();
				if (name.size() > 0)
				{
					env->push(name);
				}
