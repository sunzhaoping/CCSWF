...
namespace gameswf
{
	...
	struct as_3_function : public as_function
	{
		...
		// method_info
		int m_return_type;
		 gameswf::array<int> m_param_type;
		int m_name;
		Uint8 m_flags;
		 gameswf::array<option_detail> m_options;
		...
		membuf m_code;
		 gameswf::array<gc_ptr<except_info> > m_exception;
		 gameswf::array<gc_ptr<traits_info> > m_trait;
		...
		void	execute(gameswf::array<as_value>& lregister, as_environment* env, as_value* result);
		void	compile();
		...
		void	read_body(stream* in);
		 ...
		 gameswf::tu_string get_multiname(int index, vm_stack & stack) const;
