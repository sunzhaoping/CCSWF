...
{
	// use safe container
	static  gameswf::array<char> s_gameswf_key_to_ascii;
	static  gameswf::array<char> s_gameswf_key_to_ascii_shifted;
	...
	static void	display_glyph_records(
		...
		character* inst,
		const  gameswf::array<text_glyph_record>& records,
		movie_definition_sub* root_def)
	{
		 gameswf::array<fill_style>	dummy_style;	// used to pass a color on to shape_character::display()
		 gameswf::array<line_style>	dummy_line_style;
...
//					mat.set_scale_rotation(yscale, yscale, radians);
	...
	void edit_text_character::reset_format(as_textformat* tf)
	{
		...
		if (tf->get_member("align", &val))
		{
		 ...
		 gameswf::tu_string fontname = m_font->get_name();
	...
	const  gameswf::tu_string&	edit_text_character::get_var_name() const
	{
	...
	void	edit_text_character::set_text(const  gameswf::tu_string& new_text)
	...
	void	edit_text_character::set_text_value(const  gameswf::tu_string& new_text)
	// Set our text to the given string.
	{
		...
		if (get_var_name().size() > 0)
		{
			as_object* ch = get_parent();	// target, default is parent
			 ...
			 gameswf::tu_string var = get_var_name();
	...
	const char*	edit_text_character::html_font(const char* p, text_glyph_record& rec)
	{
		...
		while (true)
		{
			...
			if (strncmp(p, "letterSpacing=", 14) == 0)
			{
				...
				const char* end = strchr(p, '"');
				int len = int(end - p);
				 gameswf::tu_string letterSpacing(p, len);
				p += len + 1;
			}
			else
...
//		printf("%s\n", m_text.c_str());
	...
	void	edit_text_character::format_plain_text(const  gameswf::tu_string& text, text_glyph_record&	rec)
	{
		...
		const  gameswf::tu_string* textPtrToUse = &text;
		 gameswf::tu_string passwordText;
		if (m_password)
		{
			// nice big hack.
			passwordText = text;
