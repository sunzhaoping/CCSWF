...
namespace gameswf
{
	...
	struct font : public character_def
	{
		...
		const  gameswf::tu_string& get_name() const { return m_fontname; }
		void	set_name(const  gameswf::tu_string& name) { m_fontname = name; }
	...
	private:
		void	read_code_table(stream* in);
		 ...
		 gameswf::array<gc_ptr<shape_character_def> >	m_glyphs;
		 gameswf::tu_string	m_fontname;
		...
		float	m_descent;
		float	m_leading;
		 gameswf::array<float>	m_advance_table;
		// @@ we don't seem to use this thing at all, so don't bother keeping it.
		//  gameswf::array<rect>	m_bounds_table;	// @@ this thing should be optional.
		...
		struct zone_record
		{
			struct zone_data
			{
			 ...
			 gameswf::array<zone_data> m_zone_data;

			bool m_has_maskx;
			bool m_has_masky;
		};

		 gameswf::array<zone_record> m_zone_table;
	};
