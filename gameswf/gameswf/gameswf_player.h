...
namespace gameswf
{
	typedef exported_module as_object* (*gameswf_module_init)(player* player, const  gameswf::array<as_value>& params);
	...
	struct player : public ref_counted
	{
		...
		weak_ptr<root> m_current_root;
		 gameswf::tu_string m_workdir;
		string_hash<gc_ptr<character_def> > m_chardef_library;
		 gameswf::tu_string m_flash_vars;
		...
		exported_module void set_flash_vars(const  gameswf::tu_string& param);
