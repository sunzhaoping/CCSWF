...
namespace gameswf
{
	...
	struct text_character_def : public character_def
	{
		...
		rect	m_rect;
		matrix	m_matrix;
		 gameswf::array<text_glyph_record>	m_text_glyph_records;
	...
	// be changed at runtime (by script or host).
	{
		...
		float			m_leading;	// extra space between lines (in addition to default font line spacing)
		 gameswf::tu_string	m_default_text;
	...
	struct edit_text_character : public character
	{
		...
		gc_ptr<edit_text_character_def>	m_def;
		 gameswf::array<text_glyph_record>	m_text_glyph_records;
		 gameswf::array<fill_style>	m_dummy_style;	// used to pass a color on to shape_character::display()
		 gameswf::array<line_style>	m_dummy_line_style;
		rect	m_text_bounding_box;	// bounds of dynamic text, as laid out
		 gameswf::tu_string	m_text;
		bool m_has_focus;
		bool m_password;
		int m_cursor;
		float m_xcursor;
		...
		const  gameswf::tu_string&	get_var_name() const;
		void	reset_bounding_box(float x, float y);
		void	set_text_value(const  gameswf::tu_string& new_text);
		...
		void	format_plain_text(const  gameswf::tu_string& text, text_glyph_record& rec);
	...
	private:
		...
		void	set_text(const  gameswf::tu_string& new_text);
