...
namespace gameswf
{
	...
	// Handy for external binding.
	{
		...
		for (;; p++)
		{
		 ...
		 gameswf::array<with_stack_entry>	dummy_with_stack;
		...
		static  gameswf::tu_string	s_retval;
		s_retval = result.to_tu_string();
		return s_retval.c_str();
	}
	
	 gameswf::tu_string	call_method(
		as_environment* env,
		as_object* this_ptr,
		const char* method_name,
...
//		env->set_local_frame_top(local_stack_top);
	...
	as_object* action_buffer::load_as_plugin(player* player, const  gameswf::tu_string& classname,
		 const  gameswf::array<as_value>& params)
	...
	void	action_buffer::execute(
		...
		int exec_bytes,
		as_value* retval,
		const  gameswf::array<with_stack_entry>& initial_with_stack,
	...
	// The is_function2 flag determines whether to use global or local registers.
	{
		...
		// keep the latest var name(to log it if call_method failure)
		 gameswf::tu_string last_varname;

		assert(env);
		 gameswf::array<with_stack_entry>	with_stack(initial_with_stack);
		...
		for (int pc = start_pc; pc < stop_pc; )
		{
			...
			if ((action_id & 0x80) == 0)
			{
				...
				case 0x15:	// substring
				{
					...
					const  gameswf::tu_string&	str = env->top(2).to_tu_string();
					...
					// of as_value and  gameswf::tu_string...
					 gameswf::tu_string	new_string = str.c_str() + base;
				...
				{
					const  gameswf::tu_string var_string = env->top(0).to_tu_string();
				...
				{
					 gameswf::tu_string str = env->top(1).to_tu_string();
				...
				case 0x3A:	// delete
				{
					const  gameswf::tu_string varname = env->top(0).to_tu_string();
				...
				case 0x3B:	// delete2
				{
					 gameswf::tu_string varname(env->top(0).to_tu_string());
				...
				case 0x3D:	// call function
				{
					...
					if (env->top(0).is_string())
					{
						...
						const  gameswf::tu_string&	function_name = env->top(0).to_tu_string();
...
//						obj->builtin_member(env->top(1).to_tu_string(), env->top(0));
				...
				case 0x47:	// add_t (typed)
				{
					...
					{
						 gameswf::tu_string str = env->top(1).to_string();
				...
				case 0x4B:	// to string
				{
					const  gameswf::tu_string& str = env->top(0).to_tu_string();
					env->top(0).set_tu_string(str);
					break;
				}
				case 0x4C:	// dup
...
//								env->top(2).to_tu_string().c_str(),
				...
				case 0x52:	// call method
				{
					...
					as_value	result;
					const  gameswf::tu_string&	method_name = env->top(0).to_tu_string();
			...
			else
			{
				...
				case 0x8E:	// function2
				{
					...
					// @@ security: watch out for possible missing terminator here!
					 gameswf::tu_string	name = (const char*) &buffer[i];
					i += name.length() + 1;
...
//							}
	...
	static  gameswf::tu_string	s_function_names[event_id::EVENT_COUNT] =
	{
		"INVALID",		 // INVALID
		"onPress",		 // PRESS
		"onRelease",		 // RELEASE
	...
	const  gameswf::tu_string&	event_id::get_function_name() const
	{
