...
namespace gameswf
{
	...
	void	movie_def_impl::export_resource(const  gameswf::tu_string& symbol, character_def* res)
	...
	character_def*	movie_def_impl::get_exported_resource(const  gameswf::tu_string& symbol)
	...
	void	movie_def_impl::add_import(const  gameswf::tu_string& source_url, int id, const  gameswf::tu_string& symbol)
	...
	void	movie_def_impl::add_abc( gameswf::tu_string& name, abc_def* abc)
	{
		assert(m_abc == NULL);
		m_abc = abc;
		m_abc_name = name;
	...
	void	movie_def_impl::add_symbol_class(int character_id, const  gameswf::tu_string& class_name)
	{
		m_symbol_class.add(character_id, class_name);
	};

	void	movie_def_impl::add_scene(int scene, const  gameswf::tu_string& frame_name)
	{
		m_scene.add(scene, frame_name);
	};

	void	movie_def_impl::add_frame_label(int frame, const  gameswf::tu_string& frame_name)
	{
	...
	const  gameswf::array<execute_tag*>&	movie_def_impl::get_playlist(int frame_number) { return m_playlist[frame_number]; }

	const  gameswf::array<execute_tag*>*	movie_def_impl::get_init_actions(int frame_number)
	{
	...
	void	movie_def_impl::get_owned_fonts(gameswf::array<font*>* fonts)
	// Fill up *fonts with fonts that we own.
	{
		assert(fonts);
		fonts->resize(0);

		 gameswf::array<int>	font_ids;
