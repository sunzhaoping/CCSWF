...
namespace gameswf
{
	...
	const  gameswf::tu_string&	as_value::to_tu_string() const
	// Conversion to const  gameswf::tu_string&.
	{
		switch (m_type)
		{
			case STRING:
...
/*				if (get_root()->get_movie_version() >= 7)
	...
	bool as_value::find_property( const  gameswf::tu_string& name, as_value* val)
	{
		switch (m_type)
		{
			default:
	...
	bool as_value::find_property_owner(const  gameswf::tu_string& name, as_value* val)
	{
		as_value dummy;
	...
	void	as_value::set_tu_string(const  gameswf::tu_string& str)
	{
		drop_refs();
		m_type = STRING;
		m_string = str; 
		...
		m_flags(0)
	{
		...
		//
		// 1. store a tu_wstring instead of  gameswf::tu_string?
		...
		//  gameswf::tu_string?  Extra complexity.
		//
		// 3. ??
		//
		...
		// continues to work.
		 ...
		 gameswf::tu_string::encode_utf8_from_wchar(&m_string, (const uint16 *)wstr);
#else
# if (WCHAR_MAX != MAXSHORT)
# error "Can't determine the size of wchar_t"
# else
			 gameswf::tu_string::encode_utf8_from_wchar(&m_string, (const uint32 *)wstr);
# endif
#endif
	}
