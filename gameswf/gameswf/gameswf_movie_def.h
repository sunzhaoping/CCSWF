...
//				printf("signal for frame %d\n", m_loading_frame);
		...
		virtual const  gameswf::array<execute_tag*>&	get_playlist(int frame_number) = 0;
		virtual const  gameswf::array<execute_tag*>*	get_init_actions(int frame_number) = 0;
		virtual character_def*	get_exported_resource(const  gameswf::tu_string& symbol) = 0;
		...
		virtual void	add_symbol_class(int character_id, const  gameswf::tu_string& class_name) = 0;
		virtual void	add_scene(int character_id, const  gameswf::tu_string& class_name) = 0;
		virtual void	add_frame_label(int character_id, const  gameswf::tu_string& class_name) = 0;
		virtual void	add_abc( gameswf::tu_string& name, abc_def* abc) = 0;
		...
		virtual void	export_resource(const  gameswf::tu_string& symbol, character_def* res) = 0;
		virtual void	add_import(const  gameswf::tu_string& source_url, int id, const  gameswf::tu_string& symbol_name) = 0;
	...
	struct import_info
	{
		 gameswf::tu_string	m_source_url;
		int		m_character_id;
		 gameswf::tu_string	m_symbol;
		...
		import_info(const  gameswf::tu_string& source, int id, const  gameswf::tu_string& symbol):
			m_source_url(source),
			m_character_id(id),
			m_symbol(symbol)
		{
	...
	struct movie_def_impl : public movie_definition_sub
	{
		...
		hash<int, gc_ptr<sound_sample> >	m_sound_samples;
		 gameswf::array< gameswf::array<execute_tag*> >	   m_playlist;	// A list of movie control events for each frame.
		 gameswf::array< gameswf::array<execute_tag*> >	   m_init_action_list;	// Init actions for each frame.
		stringi_hash<int>		   m_named_frames;	// 0-based frame #'s
		stringi_hash<gc_ptr<character_def> > m_exports;

		// Items we import.
		 gameswf::array<import_info>	m_imports;

		// Movies we import from; hold a ref on these, to keep them alive
		 gameswf::array<gc_ptr<movie_definition> >	m_import_source_movies;

		// Bitmaps used in this movie; collected in one place to make
		// it possible for the host to manage them as textures.
		 gameswf::array<gc_ptr<bitmap_info> >	m_bitmap_list;
		...
		// for AVM2, Flash9
		 gameswf::tu_string m_abc_name;
		gc_ptr<abc_def> m_abc;
		hash<int,  gameswf::tu_string>	m_symbol_class;
		hash<int,  gameswf::tu_string>	m_scene;
		hash<int,  gameswf::tu_string>	m_frame_label;	// this is't labeled_frame !!!
		...
		virtual void	export_resource(const  gameswf::tu_string& symbol, character_def* res);
		virtual character_def*	get_exported_resource(const  gameswf::tu_string& symbol);
		virtual void	add_import(const  gameswf::tu_string& source_url, int id, const  gameswf::tu_string& symbol);
		bool	in_import_table(int character_id);
		virtual void	visit_imported_movies(import_visitor* visitor);
		virtual void	add_abc( gameswf::tu_string& name, abc_def* abc);
		const abc_def * get_abc() const{ return m_abc.get_ptr(); }
		virtual void	add_symbol_class(int character_id, const  gameswf::tu_string& class_name);
		virtual void	add_scene(int character_id, const  gameswf::tu_string& class_name);
		virtual void	add_frame_label(int character_id, const  gameswf::tu_string& class_name);
		...
		virtual const  gameswf::array<execute_tag*>&	get_playlist(int frame_number);

		virtual const  gameswf::array<execute_tag*>*	get_init_actions(int frame_number);
		void	read(tu_file* in);
		void	read_tags();
		void	get_owned_fonts(gameswf::array<font*>* fonts);
