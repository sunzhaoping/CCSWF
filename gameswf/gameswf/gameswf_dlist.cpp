...
namespace gameswf
{
	...
	character*	display_list::get_character_by_name(const  gameswf::tu_string& name)
	{
...
//					log_error("remove_display_object: no character at depth %d with id %d\n", depth, id);
	...
	// advance referenced characters.
	{
		...
		// we iterate through the copy
		 gameswf::array<display_object_info> tmp_list = m_display_object_array;
		int n = tmp_list.size();
		for (int i = 0; i < n; i++)
		{
...
/*			if (masked)
	...
	void display_list::clear_unaffected(gameswf::array<int>& affected_depths) 
	{ 
	...
	void display_list::dump( gameswf::tu_string& tabs)
	{
		tabs += "  ";
