...
namespace gameswf
{
	...
	private:
		 gameswf::array<coord_component>	m_triangle_strip;// TODO remove
		 gameswf::array<coord_component> m_triangle_list;
	};
	...
	private:
		int	m_style;
		 gameswf::array<coord_component>	m_coords;
	};
	...
	// certain error tolerance.
	{
		...
		void display( const matrix& m, const cxform& cx, const  gameswf::array<fill_style>& fills,
	...
	private:
		...
		struct layer {
			 gameswf::array<mesh*> m_meshes;  // one mesh per style.
			 gameswf::array<line_strip*> m_line_strips;

			~layer();  // delete m_meshes & m_line_strips
		};
		 gameswf::array<layer> m_layers;
	};
	...
	// information on fill and line styles.
	{
		...
		void	display( const matrix& mat, const cxform& cx, float pixel_scale, const  gameswf::array<fill_style>& fill_styles,
				const  gameswf::array<line_style>& line_styles, render_handler::bitmap_blend_mode bm) const;
		...
		const  gameswf::array<fill_style>&	get_fill_styles() const { return m_fill_styles; }
		const  gameswf::array<line_style>&	get_line_styles() const { return m_line_styles; }
		const  gameswf::array<path>&	get_paths() const { return m_paths; }
	...
	protected:
		...
		// derived morph classes changes these
		 gameswf::array<fill_style>	m_fill_styles;
		 gameswf::array<line_style>	m_line_styles;
		 gameswf::array<path>	m_paths;
	...
	private:
		...
		mutable  gameswf::array<mesh_set*>	m_cached_meshes;
	};
