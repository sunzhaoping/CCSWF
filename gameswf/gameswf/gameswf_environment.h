...
namespace gameswf
{
	...
	exported_module  gameswf::tu_string get_full_url(const  gameswf::tu_string& workdir, const char* url);
	...
	struct vm_stack : private  gameswf::array<as_value>
	{
		vm_stack() :
			m_stack_size(0)
		{
...
//			assert(index >= 0 && index < m_stack_size);
			return  gameswf::array<as_value>::operator[](index);
		}
...
//			assert(index >= 0 && index < m_stack_size);
		...
		void	push(T val) 
		{
			if (m_stack_size <  gameswf::array<as_value>::size())
			{
...
//				(*this)[m_stack_size] = as_value(val);
	...
	struct as_environment : public vm_stack
	{
		...
		as_value	m_global_register[GLOBAL_REGISTER_COUNT];
		 gameswf::array<as_value>	m_local_register;	// function2 uses this
		gc_ptr<as_object>	m_target;

		// For local vars.  Use empty names to separate frames.
		struct frame_slot
		{
			 gameswf::tu_string	m_name;
			as_value	m_value;

			frame_slot() {}
			frame_slot(const  gameswf::tu_string& name, const as_value& val) : m_name(name), m_value(val) {}
		};
		 gameswf::array<frame_slot>	m_local_frames;
		...
		as_value	get_variable(const  gameswf::tu_string& varname, const  gameswf::array<with_stack_entry>& with_stack) const;
		// no path stuff:
		as_value	get_variable_raw(const  gameswf::tu_string& varname, const  gameswf::array<with_stack_entry>& with_stack) const;

		void	set_variable(const  gameswf::tu_string& path, const as_value& val, const  gameswf::array<with_stack_entry>& with_stack);
		// no path stuff:
		void	set_variable_raw(const  gameswf::tu_string& path, const as_value& val, const  gameswf::array<with_stack_entry>& with_stack);

		void	set_local(const  gameswf::tu_string& varname, const as_value& val);
		void	add_local(const  gameswf::tu_string& varname, const as_value& val);	// when you know it doesn't exist.
		void	declare_local(const  gameswf::tu_string& varname);	// Declare varname; undefined unless it already exists.
		...
		static bool	parse_path(const  gameswf::tu_string& var_path,  gameswf::tu_string* path,  gameswf::tu_string* var);

		// Internal.
		int	find_local(const  gameswf::tu_string& varname, bool ignore_barrier) const;
