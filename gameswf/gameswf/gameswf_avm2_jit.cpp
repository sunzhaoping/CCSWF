...
namespace gameswf
{
	...
	template< typename T >
	void stack_push_back_value(  gameswf::array<as_value> & stack, T value )
	{
		stack.push_back( value );
	}

	template< typename T >
	void stack_push_back_ref(  gameswf::array<as_value> & stack, T & value )
	{
		stack.push_back( value );
	}

	typedef void ( *stack_int_function )(  gameswf::array<as_value> & stack, int value );
	typedef void ( *stack_charp_function )(  gameswf::array<as_value> & stack, char * value );
	typedef void (gameswf::array<as_value>::*push_value_function )( as_value & );
	typedef as_value & (gameswf::array<as_value>::*index_function)(int);
	...
	struct callpropvoid
	{
		static void call( const as_object *_this, const char* name, int arg_count,  gameswf::array<as_value> & stack )
		{
		...
		do
		{
			...
			switch (opcode)
			{
				...
				case 0x30:	// pushscope
				{
					...
					typedef as_value& (  gameswf::array<as_value>::*back_function )();
					jit_load( m_compiled_code, jit_this_pointer, jit_getarg( 1 ) );
					jit_this_call( m_compiled_code, (back_function)& gameswf::array<as_value>::back );

					jit_load( m_compiled_code, jit_this_pointer, var_scope );
					jit_push( m_compiled_code, jit_result );
					jit_call( m_compiled_code, (push_value_function)& gameswf::array<as_value>::push_back );
				...
				case 0x49:	// constructsuper
				{
					...
					struct constructsuper
					{
						...
						static void call(  gameswf::array<as_value> & stack, int arg_count )
						{
							as_object* obj = stack.back().to_object();
							stack.resize(stack.size() - 1);
							for (int i = 0; i < arg_count; i++)
				...
				case 0x5D:	// findpropstrict
				{
					...
					{
						static as_object * call( char* name,  gameswf::array<as_value> & scope )
						{
				...
				case 0x5E:	// findproperty, Search the scope stack for a property
				{
					...
					struct findproperty
					{
						static void call( const char* name,  gameswf::array<as_value> & scope,  gameswf::array<as_value> & stack )
						{
							as_object* obj = NULL;
							for (int i = scope.size() - 1; i >= 0; i--)
							{
				...
				case 0x68:	// initproperty, Initialize a property.
				{
					...
					struct initproperty
					{
						static void call( const char * name,  gameswf::array<as_value> & stack )
						{
				...
				case 0xD3:	// getlocal_3
				{
					...
					jit_this_call( m_compiled_code, (index_function)& gameswf::array<as_value>::operator[] );
					jit_popargs( m_compiled_code, 1 );

					//stack.push_back(val);
					jit_push( m_compiled_code, jit_result );
					jit_load( m_compiled_code, jit_this_pointer, jit_getarg( 1 ) );
					jit_this_call( m_compiled_code, (push_value_function) & gameswf::array<as_value>::push_back );
					jit_popargs( m_compiled_code, 1 );
...
#ifdef __GAMESWF_ENABLE_JIT__
		jit_load( m_compiled_code, jit_this_pointer, jit_getarg( 1 ) );
		jit_this_call( m_compiled_code, & gameswf::array<as_value>::size );
		jit_subi( m_compiled_code, jit_eax, count );
		jit_pusharg( m_compiled_code, jit_eax );
		jit_load( m_compiled_code, jit_this_pointer, jit_getarg( 1 ) );
		jit_this_call( m_compiled_code,  & gameswf::array<as_value>::resize );
		jit_popargs( m_compiled_code, 1 );
#endif
	}
}
