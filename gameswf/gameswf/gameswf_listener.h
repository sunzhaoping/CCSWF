...
namespace gameswf
{
	...
	struct listener : public ref_counted
	{
		...
		// misc
		struct clone_listener : public  gameswf::array<gc_ptr<as_object> >
		{
			clone_listener(const  gameswf::array< weak_ptr<as_object> >& listeners)
			{
		...
		exported_module void notify(const  gameswf::tu_string& event_name, const fn_call& fn);
	...
	private:
		 ...
		 gameswf::array< weak_ptr<as_object> > m_listeners;
