...
namespace gameswf
{
	...
	void	write_coord_array(tu_file* out, const  gameswf::array<coord_component>& pt_array)
	...
	void	read_coord_array(tu_file* in,  gameswf::array<coord_component>* pt_array)
...
//		memcpy(&m_triangle_strip[0], &pts[0], count * sizeof(point));
	...
	// Default constructor, for  gameswf::array<>.
		:
		m_style(-1)
	{}
...
//		memcpy(&m_coords[0], coords, coord_count * sizeof(coords[0]));
	...
	struct tri_stripper
	{
		...
		// strip during the flush.
		 gameswf::array<  gameswf::array<point> >	m_strips;
		...
		// Add two triangles to our strip.
		{
			...
			if (l0.bitwise_equal(r0) == false)
			{
				...
				for ( ; i < n; i++)
				{
					 gameswf::array<point>&	str = m_strips[i];
		...
		// under the given style.
		{
			if (m_strips.size())
			{
				...
				for (int i = 1, n = m_strips.size(); i < n; i++)
				{
					...
					const  gameswf::array<point>&	str = m_strips[i];
					assert(str.size() >= 3);	// should have at least one tri already.
	...
	void	mesh_set::display(
		const matrix& mat,
		const cxform& cx,
		const  gameswf::array<fill_style>& fills,
		const  gameswf::array<line_style>& line_styles, render_handler::bitmap_blend_mode bm) const
	...
	static void	read_fill_styles(gameswf::array<fill_style>* styles, stream* in, int tag_type, movie_definition_sub* m)
	...
	static void	read_line_styles(gameswf::array<line_style>* styles, stream* in, int tag_type, movie_definition_sub* m)
...
#define SHAPE_LOG 0
	...
	static void	debug_display_shape_paths(
		...
		const  gameswf::array<path>& paths,
		const  gameswf::array<fill_style>& fill_styles,
		const  gameswf::array<line_style>& line_styles)
	{
...
// 		// xxxxxxx
	...
	void	shape_character_def::display( const matrix& mat, const cxform& cx, float pixel_scale, const  gameswf::array<fill_style>& fill_styles, const  gameswf::array<line_style>& line_styles, render_handler::bitmap_blend_mode bm) const
