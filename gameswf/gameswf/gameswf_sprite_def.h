...
namespace gameswf
{
	...
	struct sprite_definition : public movie_definition_sub
	{
		...
		movie_definition_sub*	     m_movie_def;		// parent movie.
		 gameswf::array< gameswf::array<execute_tag*> >  m_playlist;	// movie control events for each frame.
		...
		virtual void	add_abc( gameswf::tu_string& name, abc_def* abc) { assert(0); };
		virtual void	add_symbol_class(int character_id, const  gameswf::tu_string& class_name) { assert(0); };
		virtual void	add_scene(int character_id, const  gameswf::tu_string& class_name) { assert(0); };
		virtual void	add_frame_label(int character_id, const  gameswf::tu_string& class_name) { assert(0); };
		...
		virtual void	export_resource(const  gameswf::tu_string& symbol, character_def* res) { log_error("can't export from sprite\n"); }
		virtual character_def*	get_exported_resource(const  gameswf::tu_string& sym) { return m_movie_def->get_exported_resource(sym); }
		virtual void	add_import(const  gameswf::tu_string& source_url, int id, const  gameswf::tu_string& symbol) { assert(0); }
		...
		const  gameswf::array<execute_tag*>&	get_playlist(int frame_number)
