...
//#endif	// not __GNUC__
		...
		virtual character*	add_display_object( Uint16 character_id, const  gameswf::tu_string& name, const  gameswf::array<swf_event*>& event_handlers, int depth, bool replace_if_depth_is_occupied, const cxform& color_transform, const matrix& mat, float ratio, Uint16 clip_depth, Uint8 blend_mode)
		{
			return NULL;
		}
		...
		virtual void	do_actions(const  gameswf::array<action_buffer*>& action_list) { assert(0); }

		virtual character*	clone_display_object(const  gameswf::tu_string& newname, int depth)
		{
			assert(0);
			return NULL; 
		}

		virtual void	remove_display_object(const  gameswf::tu_string& name) { assert(0); }
...
//		virtual character*	get_root_interface() { return NULL; }
		...
		void	set_name(const  gameswf::tu_string& name) { m_name = name; }
		const  gameswf::tu_string&	get_name() const { return m_name; }
