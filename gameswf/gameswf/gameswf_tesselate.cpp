...
namespace tesselate
{
	...
	// More Renderer state.
	static  gameswf::array<fill_segment>	s_current_segments;	// @@ should not dynamically resize this thing!
	static  gameswf::array<point>	s_current_path;			// @@ should not dynamically resize this thing!
	...
	// trapezoids.  Modifies the values in s_current_segments.
	{
		...
		// Peel off first.
		 gameswf::array<fill_segment>	slab;	// @@ make this use static storage
		for (int i = i0; i < i1; i++)
		{
...
namespace tesselate_new
{
	...
	struct path_part
	{
		...
		bool m_processed;
		 ...
		 gameswf::array<point> m_verts;
	};


	static  gameswf::array<path_part>	s_path_parts;
	static point	s_last_point;
	...
	void copy_points_into_array(gameswf::array<float>* out, const  gameswf::array<point>& in)
	{
		if (in.size() > 0) {
	...
	void	end_shape()
	{
		...
		for (int i = 0; i < s_path_parts.size(); i++) {
			...
			if (!pp->m_processed && pp->m_right_style != -1 && pp->m_closed && pp->m_verts.size() > 0) {
				pp->m_processed = true;
				int style = pp->m_right_style;
				 gameswf::array< gameswf::array<float> > paths;
