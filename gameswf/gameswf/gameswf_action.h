...
namespace gameswf
{
	...
	exported_module  gameswf::tu_string	call_method(
		as_environment* env,
		as_object* this_ptr,
		const char* method_name,
	...
	struct event_id
	{
		...
		event_id(id_code id,  gameswf::array<as_value>* args) :
		...
		const  gameswf::tu_string&	get_function_name() const;
	};
	...
	struct action_buffer
	{
		...
		static as_object* load_as_plugin(player* player,
			const  gameswf::tu_string& classname, const  gameswf::array<as_value>& params);
	...
	private:
		// Don't put these as values in  gameswf::array<>!  They contain
		...
		gc_ptr<counted_buffer>	m_buffer;
		 gameswf::array< gameswf::tu_string>	m_dictionary;
