...
namespace gameswf
{
	...
	// dispatch
	{
		...
		// Create local registers.
		 gameswf::array<as_value>	local_register;
		...
		if( m_compiled_code.is_valid() )
		{
			try
			{
				m_compiled_code.call<  gameswf::array<as_value>&, vm_stack&, vm_stack&, as_value* >
	...
	void	as_3_function::execute(gameswf::array<as_value>& lregister, as_environment* env, as_value* result)
	{
			...
			case 0x11: // iftrue
				...
				case 0x49:	// constructsuper
				{
					...
					// Assume we are in a constructor
					 gameswf::tu_string class_name = m_abc->get_class_from_constructor( m_method );
					 gameswf::tu_string super_class_name = m_abc->get_super_class( class_name );
...
//					as_object* basetype = stack.top(0).to_object();
				...
				case 0x66:	// getproperty
				{
					int index;
					ip += read_vu30(index, &m_code[ip]);
					 gameswf::tu_string name = get_multiname(index, stack);
				...
				case 0xA0:	// Add two values
				{
					...
					{
						 gameswf::tu_string str = stack.top(1).to_string();
	...
	// read body_info
	{
	 ...
	 gameswf::tu_string as_3_function::get_multiname(int index, vm_stack & stack) const
	{
